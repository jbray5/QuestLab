...
export default function DmDockBody({ sessionId, onSessionChange }: Props) {
  ...
  // ── Notes: autosave with a 700 ms debounce; flush on unmount ─────────────
  // The textarea shows the server's notes until the DM types; from then on it
  // shows the local draft (keyed to this session so a stale draft can never
  // leak into another session's notes).
      ...
      <div style={{ flex: 1, minHeight: 0, overflowY: "auto", padding: tab === "notes" ? 0 : "0.7rem 0.75rem", display: "flex", flexDirection: "column" }}>
        {tab === "notes" && (
          <>
            ...
            >
              <span>{session ? `Session ${session.session_number}${session.title ? ` · ${session.title}` : ""}` : ""}</span>
