...
- [x] Step 5: Verified — a second Mixamo model (three.js's X Bot) driven by the shared clips on the spike, and live on the table beside the soldiers; gate green
...
- [x] A PC with a `model_url` shows that model on the table, at its race's height, walking with the shared clips
