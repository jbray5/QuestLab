# The Severance — Campaign Companion

> DM-side reference for everything that lives outside the QuestLab HUD —
> backstories, accents, cross-PC threads, Saturday cues, open questions.
> Live document. Update as players send more in.

---

## Core facts

- **Campaign:** The Severance
- **Continent / region:** Veilmark
- **Adventure 1:** The Hearth and the Hollow
- **Session 1:** Lantern-Eve · Saturday 2026-05-23
- **Campaign UUID:** `80b6f517-d124-4fea-9435-8e727f3171a9`
- **Premise:** A powerful being is severing the shrines that anchor the
  Feywild to the Material Plane. The party is recruited to investigate
  why — and to face the moral question of whether the bonds *should*
  be saved or whether severance is mercy for a dying Feywild.

---

## Campaign motifs (the threads that braid)

1. **Missing fathers.** Parker's father is Fey and arc-relevant.
   Chelsea's Nya never knew her father — possibly the source of her
   sorcery. If Jenna leans the same way for her Warlock, this becomes
   the central campaign refrain: *who gave us what we can do, and where
   did they go.*
2. **Bonds vs. severance.** Every NPC sits somewhere on this axis.
   Yelvyne (preserve), Wenneth (sever — mercy), Halve (sever — agenda),
   Belva (no opinion, just wants her village whole).
3. **What burns must be tended.** The Lantern Circle's creed. Echoes in
   Bahamut paladins (Cory's tempered-blade-not-gilded-idol read on
   faith). Echoes in Wenneth's tree dying slowly while she hides it.

---

## The Party

### 1. Drow Warlock — *Jenna* (PC: **Sarranthia**)

- **Name confirmed:** **Sarranthia.** Last name / nicknames pending
  from Jenna.
- **Class:** Warlock 1 · **Race:** Elf (Drow) · **Background:** Wayfarer
- **Saves:** WIS, CHA · **Feat:** Lucky
- **Backstory sent:** none yet
- **Saturday hook:** her patron's voice goes silent the moment the
  shrine severs. Give her a private dread beat.
- **Open:**
  - [ ] Pick a name
  - [ ] Full backstory (use the prompt template I sent for Parker)
  - [ ] Patron flavor — open the door to her patron being tied to a
    missing parent figure (would make her the third "missing father"
    PC and lock the campaign motif)

---

### 2. Wood Elf Rogue — *Parker* (PC: **Thaelen "Thane" Amakiir**)

- **Name confirmed:** **Thaelen Amakiir** — goes by **Thane** to everyone
  except his mother. Surname taken from his father (see DM canon below).
- **Class:** Rogue 1 · **Race:** Elf (Wood) · **Background:** Wayfarer
- **Saves:** DEX, INT · **Feat:** Lucky
- **Expertise:** Sleight of Hand, Stealth
- **Backstory (Parker's words):**
  - Grew up in a secluded elven forest. Mother + stepfather, loving home.
  - **Real father disappeared when very young, no one ever explained.**
  - Left home early; works as a guide / escort, sometimes dangerous routes.
  - Wants purpose beyond survival.
  - Quiet, observant. Hard to trust, loyal once given. Values freedom.
  - Cares about: mother, stepfather, little sister, childhood friend
    **Lira**.
  - Self-taught abilities — road-learned stealth, survival, knife work.
  - Watershed: a job went wrong. He got complacent. People died. More
    careful about who he trusts now.
  - **Secret:** keeps small mementos from every place he's been.
- **DM-set canon:**
  - **His father is Fey.** Specifically: a Feywild being who came to
    live among the Wood Elves and was pulled back when the shrines
    began weakening years ago. Parker is half-Fey on his father's side.
  - The severance of the Hollowmere shrine on Saturday will hit Parker
    in his body — nausea, vertigo, and a brief sense of a *door
    closing on someone he doesn't quite remember.*
  - Whoever-is-doing-the-severing is, somehow, connected to where
    Parker's father went. Long-arc payoff.
- **Saturday hooks:**
  - Belva calls him by an elven name he hasn't heard since he was a
    child — *one* of the mementos in his pack belongs to her village,
    and she recognizes the carving.
  - During Halve's recruitment beat: Halve says one word in Sylvan that
    only Parker's father called him. Don't translate it at the table.
- **Memento confirmed:** a **broken compass** that once belonged to a
  "former mentor." Mentor's name still pending from Parker. (DM canon
  below — the compass is NOT broken, and was NOT the mentor's
  originally. Reveal much later.)
- **Open (5 questions sent):**
  - [x] **The name** — Thaelen "Thane" Amakiir
  - [x] **Father's nature** — confirmed Fey, woven into the big arc
  - [ ] The shoulder-tightening NPC archetype (kept vague is fine)
  - [ ] Whether his fey side "speaks" — if yes, how
  - [x] **The most meaningful memento** — broken compass from a former
    mentor (mentor name pending from Parker)

---

### 3. Human Sorcerer — *Chelsea* (PC: Nya)

- **Birth name:** **Nya Ashwyn** (Ashwyn = mother's name; matrilineal
  because the father was unknown)
- **Guild-name:** **Death's Shadow** (assassins' guild that raised her)
- **Class:** Sorcerer 1 · **Race:** Human · **Background:** Charlatan
- **Saves:** CON, CHA (corrected from sheet's DEX+CON) · **Feats:**
  Skilled, Tough
- **Backstory (confirmed):**
  - Mother (Ashwyn) was murdered when Nya was young.
  - Mother never knew Nya's father; Nya doesn't either.
  - Raised by an assassins' guild after her mother's death.
  - The guild gave her the name "Death's Shadow."
  - Now on the run from the guild.
- **DM-set canon (until Chelsea pins it):**
  - Her sorcerous magic comes from her unknown father. Likely Draconic.
    This makes her the second of (potentially) three "missing father"
    PCs and ties her loosely to Cory thematically (both Dragonborn /
    draconic adjacent through the father line).
- **Saturday hooks:**
  - Halve calls her name during the recruitment beat. Chelsea picks
    which: "Nya Ashwyn" (intimate), "Death's Shadow" (threatening),
    or both in sequence. Default if no answer: both.
  - The recruiter "knows her past" but does *not* pay off in Session 1.
    He's testing how she reacts to being seen, not exposing her yet.
- **Open (5 questions sent):**
  - [ ] Sorcerous origin (Draconic recommended)
  - [ ] Why she actually fled the guild (specific watershed)
  - [ ] What "Death's Shadow" denoted (role / résumé)
  - [ ] Mother's fate — twist? (guild internal? lover? rival?)
  - [ ] Which name Halve says

---

### 4. Dragonborn Paladin — *Cory* (PC: Creed Ashmantle)

- **Class:** Paladin 1 · **Race:** Dragonborn · **Background:** Noble
- **Saves:** WIS, CHA · **Feat:** Skilled
- **Alignment:** Lawful Neutral
- **God:** **Bahamut** *(confirmed)*
- **Backstory (Cory's words):**
  - Born in a radiant temple; rose through priesthood young.
  - Booming voice, unwavering discipline. Inspired awe.
  - Divine power first answered as *warmth* — healing, blessing,
    shielding.
  - **Uncovered corruption** in the high clergy; spoke out; was
    branded heretic, stripped of vestments, cast out.
  - The god did not abandon him. The church did.
  - Now a paladin **bound by oath, not by institution.**
  - Walks the world as blessing AND warning. Aids temples that serve
    the people; dismantles those that rule them.
  - Kneels in prayer not for forgiveness but for clarity.
  - Wants to restore his own faith in people; prove not everyone is
    self-interested.
  - Flaw: lack of trust.
  - **Plays the lute** (background color).
- **DM-set canon:**
  - The temple Cory left was a **Bahamutan** temple corrupted from
    within. The god is fine; the institution was the rot.
  - His liege (House Ashmantle) sent him to Hollowmere to investigate
    Lantern-Eve. The liege has interest in the severances — agenda
    unknown to Cory.
  - Cory respects the gods but holds mortal clergy to a *harsher*
    standard than any sinner.
- **Saturday hooks:**
  - Yelvyne clocks his "Bahamutan bearing" the moment he walks into
    the shrine. She extends professional courtesy.
  - But: Cory's church-corruption history means he will *judge*
    Yelvyne and the Lantern Circle based on what they actually do for
    the people of Hollowmere — not on her credentials. The moral
    choice at session's end will land in him as: "Is this priest
    worthy of my sword arm, or my judgment?"
  - During Halve's recruitment beat: Halve mentions, by name, the
    high cleric Cory exposed. Halve says it with respect. Cory has
    to decide what that respect means.

---

### 5. Aasimar Druid — *Hayley* (PC: Willa Thornwood)

- **Class:** Druid 1 · **Race:** Aasimar · **Background:** Sage
- **Saves:** INT, WIS · **Feat:** Magic Initiate (Druid)
- **Alignment:** Chaotic Neutral
- **Primal Order:** Warden
- **Backstory sent:** none yet
- **DM-set canon (from earlier prep):**
  - As a Sage, she has read about the Lantern Circle. She volunteers
    *one* prepared fact at the tavern when the topic comes up — DM
    makes it canon on the fly.
  - Aasimar lineage — celestial blood. May feel the shrine's silver
    light more keenly than the others.
- **Saturday hooks:**
  - Yelvyne treats her with quiet reverence (an Aasimar walking into
    the Lantern Shrine is a small religious event for the steward).
  - When the shrine severs, Willa's *Healing Hands* trait could play
    a critical role — Yelvyne may ask her to lay hands on the dying
    shrine itself. Whether that does anything is up to Hayley's roll.
- **Open:**
  - [ ] Full backstory (use prompt template)
  - [ ] The one Lantern Circle fact she "remembers" volunteering

---

## NPC extras (beyond the in-app card)

### Master Halve (the recruiter)

- **The big trick:** he calls out one detail per PC that *only that PC
  would know.* Order of cuts on Saturday:
  - **Drow Warlock:** something about her patron (the silence she's
    about to feel)
  - **Parker:** the Sylvan word his father called him
  - **Nya:** her birth name OR Death's Shadow (Chelsea picks; default
    both)
  - **Creed:** the name of the corrupt high cleric Creed exposed
  - **Willa:** a verse from a Lantern Circle text she's read but
    shouldn't have access to as an outsider
- **He leaves the loft BEFORE combat.** PCs returning find his cloak
  on the chair, old coin stacked on Belva's bar. No body, no scent.
- **Suspicion at session end: 100. Reveals: 0.**

### Wenneth (Dryad)

- The corrupted Dryad in Saturday's combat is **Hellbrim** — a friend
  of Wenneth's. Wenneth recognizes her. Read in Wenneth's voice in the
  PCs' heads during combat:
  > *"That is Hellbrim. We grew together for two centuries. Do not
  > hate her for what is no longer her."*
- Wenneth's slow-burn corruption arc begins Session 1. She wilts when
  the shrine severs. Each subsequent session she changes physically
  unless the PCs find a way to halt it.
- **Hidden tie-in to Parker:** Wenneth may have known Parker's father.
  Don't reveal Session 1. Hold for Session 3 or later.

### Belva

- Her **scarred ear** is the story she won't tell. Reserve for a later
  session when a PC presses hard enough.
- The old coin Halve paid in is from a **dead kingdom** Willa might
  recognize from her Sage reading.

### Priestess Yelvyne

- After combat, she offers the **Lay-of-Lanterns** ritual to reseal
  the shrine for one more night. If Hayley's player wants Willa to
  participate (Healing Hands as the catalyst), it adds narrative
  weight but mechanical impact is DM's call.

---

## Saturday cue card (the one I'll keep on my phone)

| Moment | What to do |
|---|---|
| PCs enter the Hearth | Read opening read-aloud (full text in runbook). Don't skip the silver-light line. |
| Halve enters | Five individual cuts, one per PC. Use the table above. |
| Halve exits | He says: *"There's something at the shrine that wants seeing. Walk over with the Priestess — I'll catch up."* Then he goes upstairs to the loft. |
| Shrine severs (Act 2) | Trigger the cuts: Warlock dread, Parker physical pull, Wenneth's scream. |
| Combat starts (Act 3) | Use the encounter row "Hold the Hearth." Wenneth corrupts and IS the dryad. Hellbrim line via dying shrine. |
| Combat ends | Yelvyne arrives wounded. Wenneth roots into a dead, leafless tree as she dies — one last corrupted-lucid line, no clean plea. Master Halve found gone. |
| Cliffhanger | Moral choice: ritual to reseal vs. let it sever. Defer until dawn. |

---

## Open items (player-side)

| Player | Pending |
|---|---|
| Jenna (Sarranthia) | Last name/nicknames, backstory, patron flavor |
| Parker (Rogue) | Name + 4 of 5 prompts |
| Chelsea (Nya) | 5 prompts |
| Cory (Creed) | Nothing — god confirmed, backstory complete |
| Hayley (Willa) | Backstory + the one Lantern Circle fact |

---

# 🔒 DM-ONLY CANON — PLAYERS DO NOT READ

Everything below is the backstage truth of the campaign. The PCs will
learn fragments of this over many sessions, but the whole picture is
DM-only. Do NOT share this section with players.

---

## Master Halve — full backstory

**Pronunciation:** *HAHV.* Silent L, long A. Like "behalf" minus the
"be." Crisp consonants, soft V, almost swallowed.

### The history

~300 years ago, in a region long since renamed and forgotten, was
**Veilglass.** Its Feywild bond was the strongest in the world. The
silver light over Veilglass burned so bright the village had no nights
— only a permanent dusk. Children born during that light were *changed:*
split shadows, eyes that didn't track right, things said in their sleep
that came true. The Feywild was too close. The cost was paid in bodies.

Halve was one of those children. His mother was a high priestess of the
Veilglass Lantern Circle (the order that maintained the bonds). His
father was a Feywild being who came through the bond, married into the
community, and stayed. **Halve is half-Fey.**

In Halve's young manhood, a faction of priest-philosophers argued the
bond was killing the region and pushed for deliberate severance. His
mother fought them. The dissidents won — by force. They performed the
**first deliberate severance in recorded history.**

The backlash killed Halve's mother. His father was pulled back through
the bond at the moment of severance, never seen again. Halve, anchored
on both sides, was the only survivor in the close perimeter. The snap
pulled him in two directions and **did not break him** — but did not
leave him whole, either.

After the severance, Veilglass healed. Slowly. The corrupted children
stopped being born. The Feywild withdrew to a safe distance. The land
became ordinary. Beautiful in a quiet way.

The priest-philosophers were right.

Halve has been alive ever since. **He doesn't age** — half his soul went
out with the Feywild and is somewhere else, leaving the half that
remains in a kind of stasis. He has been **a man halved** for three
centuries.

### What he believes

> *"The bond is a parasite. Every region where it persists, the Feywild
> bleeds in and the mortal world rots out. I have seen this pattern five
> times in my life. The only thing that has ever worked is severance.
> The only thing."*

He's not lying. He's also not provably right. The campaign's central
tension is that his thesis MIGHT be true — and the PCs will never get a
clean answer.

### Why he chose these five PCs

Halve selects targets based on **fey-adjacency.** A clean severance
needs people with one foot on the Material side who can still be heard
by the Feywild while it withdraws.

- **Drow Warlock (Jenna).** Her patron is fey-adjacent. The patron-bond
  itself is the relevant tie.
- **Wood Elf Rogue (Parker).** Half-Fey paternally. Halve may not know
  Parker's father was an earlier recruit of his.
- **Nya (Chelsea).** Her sorcery comes from a fey-touched paternal line.
  Halve suspects but hasn't confirmed.
- **Creed (Cory).** The corrupt Bahamutan cleric Creed exposed was
  secretly extending bonds for personal gain. **Halve identified him
  decades ago and tipped Creed's investigation.** Creed thinks he
  uncovered it himself.
- **Willa (Hayley).** Aasimar. Celestial-blooded. She can negotiate with
  the Feywild without being corrupted by it. **The keystone of his team.**

### Hellbrim

**NOT on the field Saturday.** The combat Dryad is Wenneth, not
Hellbrim.

Hellbrim was Halve's previous recruit, ~20 years ago, in a different
region. A fey woman who volunteered as the "negotiator" during a
severance attempt. It went wrong. She came back corrupted. She's
wandered between regions ever since, drawn to active Feywild bonds.

**She passed through Hollowmere months ago** and seeded the corruption
in Wenneth's grove — that's why the Chapel Oak has been yellowing all
year. Hellbrim is the reason the severance hits Hollowmere so hard, so
fast — Wenneth was already half-corrupted when the bond snapped.

The Saturday combat line *"That is Hellbrim. We grew together for two
centuries. Do not hate her for what is no longer her"* is **Wenneth's
self-recognition** — she's naming what she has just become, not pointing
at a separate creature. "We grew together" means shared corruption fate.

Hellbrim herself is alive somewhere. Future-session antagonist —
wandering corruption-spreader, much harder to fight than Wenneth.

### Halve's Saturday behavior, decoded

| Beat | What's really happening |
|---|---|
| Sends the letters with old coin | The coin is **Veilglass mint**, ~300 years old. Willa may recognize it on a high check. |
| Enters the Hearth, performs five cuts | This is **calibration.** He's confirming each PC has the fey-tie he believed. If any failed, he'd walk away and try in 20 years. |
| Exits with *"I'll catch up"* | He never intends to. He goes to Belva's loft. |
| Performs the ritual in the loft | He holds his **Veilglass anchor token** (the second piece of his soul, in physical form) and *listens.* Through the PCs' resonance at the shrine, he asks the Feywild to withdraw. |
| Disappears before combat | He cannot face Hellbrim's victims again. He calls it "thresholds." It's cowardice. He won't admit it. |
| Leaves coin on Belva's bar | Paying for what he took. Also: severing his account with Belva — he doesn't expect to return. |

---

## How severance actually works — mechanic

The bond between the Material Plane and the Feywild is **metaphysical**,
but every region has a **physical anchor** — a shrine, an oak, a stone,
a spring. The Hollowmere anchor is the **Lantern Shrine** (and
secondarily Wenneth's Chapel Oak). The shrine isn't the bond — it's the
*handle* the bond holds.

**The bond cannot be severed by force.** You can burn the shrine down
and the bond persists; another anchor will form. The bond can only be
severed by **negotiation** — somebody has to *speak* to the Feywild and
ask it to withdraw. The Feywild has to *consent.*

That requires three things working together:

1. **A speaker.** Someone half-anchored — one foot on each side. Halve.
   Only a handful of half-fey beings in the world fit this. Halve is the
   only one currently doing it.
2. **Witnesses.** Mortals with enough fey-adjacency that the Feywild
   recognizes them as "people who count" on this side. Their presence
   forms a resonant circle. The five PCs are this.
3. **A focal object.** Something physical that resonates with the bond.
   Usually the local shrine. Halve also carries his Veilglass anchor
   token — he pairs his token with the shrine for two anchors instead of
   one.

### The three phases

| Phase | When | What happens |
|---|---|---|
| **Snap** | Lantern-Eve, ~9-10 PM | Halve's ritual asks the Feywild to withdraw. The Feywild signals consent by extinguishing the shrine-flame. From the PCs' perspective: light goes out, Belva drops her tankard, Wenneth screams. **The bond is closing, not closed.** |
| **Retraction window** | ~10 PM until dawn | The Feywild pulls back slowly. The bond is ancient and tangled; untangling takes hours. **The bond can still be re-asserted from the mortal side during this window.** Corrupted fey appear — their fey-essence couldn't retract because Hellbrim's residue had damaged it. They're stranded mid-pull. |
| **Dawn lock** | Sunrise | Withdrawal completes. The bond is permanently gone. No counter-ritual works after this point. |

### Why Wenneth turns

Halve's ritual pulls **healthy** fey-essence home. Corrupted essence is
too broken to retract — it stays behind, severed from its source
mid-rot. **Stranded fey-essence becomes corrupted creatures.**

So most of Hollowmere's fey (the unseen, the deep-grove) went home
safely. Wenneth, the Blossom-Seller, the Chapel Oak pixies were too
infected by Hellbrim's prior touch to retract. They turn instantly
because there's nothing left on the Feywild side to anchor them.

From Halve's view: he saved ~70% of the local fey from corruption that
would have eventually consumed all of them. From Yelvyne's view: he
created the corruption by severing prematurely. **Both views are
coherent. Neither is provably right.**

---

## Yelvyne's Lay-of-Lanterns — the counter-ritual

An old Lantern Circle rite, kept for exactly this scenario: an
unauthorized severance. Yelvyne has never performed it. She learned it
from her predecessor and never expected to need it. She's terrified
she'll fumble.

The ritual needs:
1. **The shrine itself** (intact, just dark)
2. **A vessel** — someone celestial- or fey-blooded who can amplify the
   call across both planes. **Willa (Aasimar) is the natural vessel.**
3. **The PCs' knowing consent.** Unlike Halve's ritual where they
   participated unwittingly, this time they have to **agree out loud.**

If successful, the Feywild **reverses its withdrawal** and pours back
through the bond. The shrine relights at dawn.

---

## The three paths — Saturday's moral choice

### Path A — Reseal (Yelvyne's offer)

> *"We catch the light before it leaves. The bond holds."*

**Result:** Lay-of-Lanterns succeeds. Bond reasserted. Shrine relights.

**Cost:**
- **Wenneth and the corrupted fey stay corrupted permanently.** They
  were too damaged to retract; reasserting the bond doesn't fix them.
  They become ongoing threats in the grove.
- **The bond comes back tainted.** Whatever was wrong with Hollowmere's
  bond (the decay Halve was responding to) is still there. In 5-10
  years (campaign-time), Hollowmere will face a worse severance crisis
  with more corrupted fey.
- Halve will eventually return, more methodical, less polite.

### Path B — Let it sever (Wenneth's plea)

> *"Let it go. The Feywild does not need a leash."*

**Result:** No counter-ritual. Retraction completes at dawn. Bond is
permanently gone.

**Cost:**
- **The corrupted fey are stranded forever.** They live as monsters in
  the woods around Hollowmere. The PCs may have to put them down in
  later sessions. Mercy killings.
- **Hollowmere becomes ordinary.** No more silver-light festivals. No
  more Feywild miracles. The town loses something culturally and
  economically.
- **Yelvyne breaks.** Her purpose was the bond. She may leave the
  village. May not survive the year.
- Halve will eventually approach the party for the NEXT region. They've
  passed his test.

### Path C — The wild card

The PCs may try to do something unscripted. The Feywild is in
deliberation during the retraction window — they could attempt to
**negotiate directly** with it, instead of choosing between Halve's
withdrawal and Yelvyne's full reassertion.

This is the campaign's potential third path — the one Halve thinks is
delusional and Yelvyne is too institutional to imagine. Possibilities:
- A partial withdrawal — the bond retracts from corrupted areas but
  stays in healthy ones
- Ask the Feywild to heal itself first
- Ask Wenneth (in her dying clear moment) what *she* wants

Not guaranteed to work. **You, the DM, decide what's possible.** The
door is open. This is the option that grows into the campaign's main
thrust if the PCs find their way to it.

---

## Halve after Saturday

He doesn't know what happened. He's miles away by dawn.

- If the PCs **resealed**, Halve senses the bond still humming after a
  few days. He grimaces, drinks something, accepts the team failed his
  test this round. He comes back in a few months, different approach.
- If they **let it sever**, he won't reach out for months — recruiting
  too fast looks like manipulation. Maybe a year. Then he writes a
  second letter, this one warmer.
- If they **took Path C** and something unexpected happened, **he will
  come find them in person.** He has questions. This is the only path
  that makes Halve break his usual remove and engage directly.

---

## Thane (Parker's PC) — DM canon

**Reveal timing:** Almost NONE of this lands in Session 1. Most lands in
sessions 3-10 across a long campaign of cosmological intrigue.

### His father — *Cael Amakiir*

A Fey being who came through the Feywild bond into Thane's mother's
forest community ~30 years ago. Took the Wood Elf surname Amakiir on
arrival.

**Cael was originally one of Halve's recruits** — for an older region's
severance, ~40 years ago. Cael was meant to be a fey-anchored speaker.
**At the last moment he defected.** Fled across regions, hid his
fey-nature, married into the Wood Elf community, fathered Thane.

When Thane was ~5, Halve found Cael. Not violently — just *reminded* him
the original severance was still pending and his fey-essence was tied
to the older region regardless. Cael chose neither — walked into the
woods one autumn morning and left.

**What actually happened:** Cael is alive. Halve found him a second time
and is *keeping* him — too valuable a half-anchored speaker to waste.
Cael has been a prisoner-asset somewhere in Veilmark for ~25 years.

**Campaign-arc hook:** finding and freeing (or losing) Cael is a
potential climax. Don't lock the outcome yet.

### The compass — NOT broken

Cael made it himself. A half-fey craftsman's gift to his wife in the
year Thane was born. It has two needles:

- **Brass needle** — always points to the nearest active Feywild anchor
  (shrine, strong oak, place where the planes touch).
- **Silver needle** — Thane has never seen it move. Buried, dim, almost
  invisible against the brass. **It points to Cael.** Silver-dim when
  Cael is hidden or distant; faintly glowing when Cael is near. Goes
  dark when Cael dies.

For 20 years the brass needle has spun wildly — the original Feywild
anchor Cael was bonded to was already half-severed; the compass has had
no clean target, so it twitched and spun. **Thane thinks it's broken.**
It isn't.

### The mentor

10 years after Cael vanished (Thane ~15), an old road-guide took an
interest in him. The mentor was **another defector from Halve's
network** — a former witness who refused his role decades earlier.

The mentor never told Thane the truth about Cael. He taught Thane to
survive on the road and quietly trained him for things he didn't yet
need to know. Died when Thane was ~17, old age. Gave Thane the compass
on his deathbed — the mentor had been holding it for years on Thane's
mother's behalf.

Mentor's last words (rough — Parker can rewrite when he names the
mentor):

> *"It pointed toward your father once. Now it spins. Don't ask it where
> he went — it won't tell you, and you won't like the answer."*

Thane thinks the mentor stole it from some forgotten place. **He does
not know it was made by his father.** Reveal in session 3-4 at earliest.

### Saturday — the compass beats (subtle, no reveal)

DM-side awareness only. Thane should *not* understand any of this
tonight. The prop is alive — quietly. Future sessions amplify the
signal.

| Moment | Compass behavior | At-the-table |
|---|---|---|
| Walking into Hollowmere | Brass needle steadies for the first time in 20 years | **Don't mention.** DM-only note. |
| Halve's cut to Thane (the Sylvan word) | Needle stops spinning and points at Halve for one heartbeat | **Don't mention** unless Parker is roleplaying carefully checking his pocket. If he does, describe one line: *"It's pointing somewhere it's never pointed before. You're not sure where."* No more. |
| Severance — shrine goes dark (Act 2) | Single pulse of heat in Thane's pocket. The needle slams toward the dying shrine and then **goes utterly still.** No spin. | **This is the only beat we let surface naturally.** If a scene-beat lets you describe pocket-feel, say: *"Something in your pocket — the compass? — feels warm for half a heartbeat, then cold."* Don't elaborate. Don't let Thane investigate during the immediate chaos. |
| Combat (Act 3) | Compass stays dead. | Don't mention. |
| Aftermath, **if reseal (Path A)** | Brass needle wakes back up. Spins lazily, the same as it always has. | Optional, only if Parker checks: *"It's spinning again. Same broken compass."* |
| Aftermath, **if let sever (Path B)** | Brass needle stays dead. Silver needle — which Thane has never noticed — gives **one faint pulse** and goes dark. | **Don't mention at all.** This is a campaign-arc seed for the DM. Cael, somewhere in Veilmark, just felt his son's compass go cold. Pay it off in session 3+. |
| Aftermath, **if Path C** | DM call based on what happened. | DM call. |

**Sylvan word Halve uses:** suggested **"Aelim"** — Wood-Sylvan for
"small light" or "found-flame," a parent's tenderness-word for a child.
Parker can override.

### Defectors from Halve's network — campaign arc

Cael and the mentor were both Halve recruits who defected. **There are
others.** Scattered across Veilmark, hidden in mortal communities,
living quiet lives. They form a loose underground that knows what Halve
does and refuses to participate. Some are old, some are dying, some are
broken.

**Long-arc seed:** The PCs eventually find this network (Session 6-8
range). Allies for any of the three Saturday paths. Enemies if the PCs
side with Halve. A potential power-base for whatever the PCs choose to
do with the bonds across Veilmark.

---

## Sarranthia (Jenna's PC) — DM canon

### The setting fact: the bright world hates the Drow

The fey-blending has made Veilmark **bright** — the shrines pour silver
light, the world has no true dark. For most peoples that's wondrous.
For the Drow it's hostile: a dark-adapted people with no shadow to live
in, physically and socially uncomfortable in the perpetual glow.

Worse — when shrines started going dark, the prejudice found its
scapegoat. *"Creatures of the dark. Of course the Drow want the light
gone. Of course they're behind the severances."* Across the campaign,
frightened towns blame the Drow for the severances. It is a **false
accusation** — the real severer is a half-fey man named Halve — which
makes the scapegoating tragic dramatic irony the players (who know the
truth) will feel keenly.

### The irony at Sarranthia's core

Sarranthia is the one person at the table who **most needs the bonds to
hold.** Her warlock power flows from a patron *inside the Feywild.*
Severance cuts that bond — and her magic dies with it. The world
assumes the Drow wants the dark; the truth is she is defending the
light harder than anyone, because the light is her lifeline.

She keeps her **patron's identity secret** — not for mystery's sake, but
survival: a Drow with a *visible* tie to the Feywild, in a world primed
to blame Drow for Feywild trouble, is a lightning rod.

### How it plays at the table

- **Saturday's moral choice becomes a vise for her.** The session ends
  on sever-vs-preserve. Sarranthia has a fierce, secret stake in
  *preserve* — and cannot say why without naming her patron. The most
  pro-preservation voice in the room is the one who can't explain
  herself. Pure roleplay tension; costs the DM nothing to set up.
- **Halve's recruitment is a personal knife.** Halve recruits
  fey-adjacent people for clean severances. Sarranthia's fey-adjacency
  *is* a patron severance would kill. He is asking her to help cut the
  branch she stands on. Canon: **Halve knows, and considers her power
  acceptable loss** — consistent with his 300-year certainty. When she
  realizes it, it's a betrayal with her name on it.
- **Scapegoat danger is world-level, not party-level.** Let the *world*
  distrust her; keep the other PCs (five first-timers) out of it early.

### Pact origin — PENDING from Jenna

Jenna is deciding *why* Sarranthia entered the pact. Options on the
table, tilted by the setting:
- **Protection / political power** (thematically tightest) — *"Being a
  Drow in the bright world meant being prey. I refused to be prey."*
  The pact as armor against the prejudice; the cruel irony is it ties
  her to the very thing the prejudice falsely accuses her of.
- **Bartering for a life** (most sympathetic, most fey) — Archfey
  bargains are crooked; she may not fully grasp what she agreed to, or
  the price isn't done being paid.
- **Greed / fame** — viable if Jenna wants a real flaw to grow out of;
  the harder road.

Fold the final answer in here once Jenna responds.

### Saturday PS (already written)

Halve's letter PS, kept verbatim and **ungendered** until Jenna picks
the patron's gender: the "voice" that has carried her far will go quiet
when the shrine severs. With this canon, that PS now cuts deeper — Halve
is telling a Drow he knows her single most dangerous secret: a Drow
bound to the Feywild.

---

## Letter PSes — tracking

Halve's recruitment letters each carry a private postscript: the one
thing Halve wants that PC to know he sees. Cosmology assumed: the bonds
are **fey/material only** (a severance cannot touch a divine tie).

- **Sarranthia** — done (see "Saturday PS" above).
- **Thane** — done (workshopped separately).
- **Willa — LOCKED:**

  > *P.S. — There are five letters; four are invitations. Yours is the
  > hinge the night turns on — I built the others around you. Before
  > dawn, every frightened soul in Hollowmere will try to press their
  > answer into your hands. But the hands are yours, and so is the
  > choosing. Let no one — not the priestess, not even your companions —
  > make it for you.*

  Halve will not push her to reseal *or* sever — his wound is
  Veilglass-by-force. The PS only ensures the choice is genuinely hers,
  uncoerced. A variant exists with a sharper seed of doubt ("ask what
  the light costs") if a more manipulative Halve is wanted.

- **Creed — recommended draft; DM will walk it through with Cory
  before the session:**

  > *P.S. — Four of these letters went to people the Feywild can hear.
  > Yours did not — and tonight that is exactly why I need you. The
  > others are the voices; you are the ground they stand on. When the
  > night pulls, do not move.*

  Creed is the team's Material-side anchor (no fey-tangle = the stable
  point). Pairs with the "Hold the Hearth" chokepoint role.
